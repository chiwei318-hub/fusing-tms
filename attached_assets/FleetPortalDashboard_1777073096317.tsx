    ...
    <div style={S.root}>
      ...
      <header style={S.header}>
        <div>
          ...
          <div style={S.sub}>
            {FLEET_TYPE_LABEL[(user as any)?.fleet_type ?? "owner"] ?? "車隊"} ·
            損益管理中心
          </div>
        </div>
        ...
        {tab==="self_orders" && (
          <div>
            <div style={{ display:"flex", gap:8, marginBottom:14 }}>
              <Btn color="#6366f1" onClick={() => setShowOrderForm(true)}>
                ＋ 新增自接單
              </Btn>
        ...
        {tab==="costs" && (
          <div>
            <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"center" }}>
              <Btn color="#6366f1" onClick={() => setShowCostForm(true)}>
                ＋ 登記成本
              </Btn>
              ...
              <div style={{ ...S.card, marginBottom:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  ...
                  <FField label="金額" value={costForm.amount} type="number"
                    onChange={v => setCostForm(p=>({...p,amount:v}))} />
                  <FField label="說明" value={costForm.description}
                    onChange={v => setCostForm(p=>({...p,description:v}))} />
                  <FField label="收據號碼（選填）" value={costForm.receipt_no}
                    onChange={v => setCostForm(p=>({...p,receipt_no:v}))} />
                </div>
                <div style={{ display:"flex", gap:8, marginTop:14 }}>
                  <Btn color="#1e293b" textColor="#64748b"
                    onClick={() => setShowCostForm(false)}>取消</Btn>
                  <Btn color="#6366f1"
