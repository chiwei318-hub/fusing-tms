...
export default function FinanceDashboard() {
  ...
  const unpaidPayroll = payrolls.filter((p: any) => p.status !== "paid").length;
  const unpaidFleet   = fleetPayables.filter((f: any) => f.status !== "paid").length;
  ...
  const calcFleet = useMutation({
    ...
    onSuccess: (d: any) => {
      ...
      toast({ title: `✅ 車隊應付款計算完成，共 ${d.count ?? 0} 筆` });
    },
  });
      ...
      <div style={S.content}>
          ...
          <div>
            ...
            <Tbl
              ...
              rows={payrolls.map((p: any) => [
                p.driver_name, String(p.total_trips),
                fmt(toNum(p.gross_pay)), fmt(toNum(p.withholding_tax)),
                ...
                p.status!=="paid"
                  ? <Btn size="sm" color="#064e3b" textColor="#4ade80"
                      onClick={() => openPay("payroll", p.id, p.driver_name, toNum(p.net_pay))}>付款</Btn>
        ...
        {tab==="fleet" && (
          <div>
            ...
            <Tbl
              headers={["車隊","趟次","應付","扣繳","健保補費","實付","狀態","操作"]}
              rows={fleetPayables.map((f: any) => [
                f.fleet_name, String(f.trip_count ?? "—"),
                fmt(toNum(f.gross_amount)), fmt(toNum(f.withholding_tax)),
                fmt(toNum(f.nhi_supplement)),
                <strong style={{ color:"#f59e0b" }}>{fmt(toNum(f.net_payable))}</strong>,
                <SBadge status={f.status} />,
                f.status!=="paid"
                  ? <Btn size="sm" color="#064e3b" textColor="#4ade80"
          ...
          <div>
            <div style={{ display:"flex", gap:8, marginBottom:14, alignItems:"center" }}>
              ...
              <div style={{ fontSize:12, color:"#475569" }}>
                本期 {invoices.length} 張 · 總額 {fmt(invoices.reduce((s:number,i:any)=>s+toNum(i.total_amount),0))}
              </div>
            </div>
            <Tbl
              headers={["發票號碼","客戶","統編","未稅","稅額","含稅","狀態","操作"]}
              rows={invoices.map((inv: any) => [
                <span style={{ fontFamily:"monospace", fontSize:12 }}>{inv.invoice_no}</span>,
                inv.customer_name ?? "—",
                inv.customer_tax_id ?? "一般",
                fmt(toNum(inv.amount)), fmt(toNum(inv.tax_amount)),
                <strong style={{ color:"#f59e0b" }}>{fmt(toNum(inv.total_amount))}</strong>,
                <SBadge status={inv.status==="void" ? "void" : "issued"} />,
                inv.status!=="void"
                  ? <Btn size="sm" color="#3f0000" textColor="#f87171"
                      onClick={() => { if(confirm("確定作廢？")) voidInvoice.mutate({ id:inv.id }); }}>
                      作廢
                    </Btn>
