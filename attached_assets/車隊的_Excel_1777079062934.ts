 ...
 *
 * 執行：npx ts-node src/scripts/importXiaoYangFleet.ts
 * 或在 Replit Shell：npx ts-node artifacts/api-server/src/scripts/importXiaoYangFleet.ts
 */
...
// ── 車輛資料 ──────────────────────────────────────────────────
// 民國年轉西元：115 = 2026, 114 = 2025, 116 = 2027
...
async function main() {
  ...
  const driverCols = [
    ...
    `ALTER TABLE drivers ADD COLUMN IF NOT EXISTS driver_type TEXT DEFAULT 'affiliated'`,
  ...
  console.log(`📋 車隊：${fleetName}（ID: ${fleetId ?? "未找到，將設為 NULL"}）\n`);
    ...
    try {
      const { rows } = await pool.query(`
        ...
        ON CONFLICT (phone) DO UPDATE SET
          shopee_worker_id = EXCLUDED.shopee_worker_id,
        ...
        RETURNING id, name
      `, [
        d.name, d.phone, d.shopee_id, d.id_number,
        d.birthday, d.address, d.driver_type, fleetName,
      ]);

      // 同步寫入 shopee_drivers
      await pool.query(`
        INSERT INTO shopee_drivers (shopee_id, name, phone, status)
        VALUES ($1,$2,$3,'active')
        ON CONFLICT (shopee_id) DO UPDATE SET
          name  = EXCLUDED.name,
          phone = EXCLUDED.phone
      `, [d.shopee_id, d.name, d.phone]).catch(() => {});

      const tag = d.driver_type === "external" ? "（外車）" : "";
      console.log(`  ✅ ${d.name} ${tag}工號:${d.shopee_id} → driver ID: ${rows[0]?.id}`);
      driverCount++;
    } catch (e: any) {
    ...
    try {
      await pool.query(`
        ...
        ON CONFLICT (plate_no) DO UPDATE SET
          ...
          note              = EXCLUDED.note
      `, [
        fleetId, fleetName, v.plate_no, v.brand, v.model,
        v.manufactured,
        v.max_load_t * 1000,      // 公噸→公斤
        v.gross_weight_t * 1000,
        v.insurance_expiry, v.inspection_expiry,
        v.color, v.is_external, v.note ?? null,
      ]);

      const expired = v.inspection_expiry && new Date(v.inspection_expiry) < new Date()
        ? "⚠️ 驗車逾期！" : "";
      console.log(`  ✅ ${v.plate_no} ${v.brand} ${v.max_load_t}T ${expired}`);
      vehicleCount++;
    } catch (e: any) {
  ...
  for (const v of VEHICLES) {
    if (v.inspection_expiry) {
      ...
      if (d < today) console.log(`  🔴 ${v.plate_no} 驗車已逾期！（${v.inspection_expiry}）`);
      else if (d < soon) console.log(`  🟡 ${v.plate_no} 驗車即將到期（${v.inspection_expiry}）`);
    }
    if (v.insurance_expiry) {
      const d = new Date(v.insurance_expiry);
      if (d < today) console.log(`  🔴 ${v.plate_no} 保險已逾期！（${v.insurance_expiry}）`);
      else if (d < soon) console.log(`  🟡 ${v.plate_no} 保險即將到期（${v.insurance_expiry}）`);
    }
  }

  // ── 7. 結果摘要 ──────────────────────────────────────────────
  console.log(`\n✅ 匯入完成！`);
  console.log(`   司機：${driverCount} 筆`);
