...
export function createFleetSystemRouter() {
  ...
  router.patch("/fleets/:id/type", async (req: Request, res: Response) => {
    try {
      ...
      const { rows } = await pool.query(`
        ...
        WHERE id = $4
        RETURNING *
      `, [fleet_type, monthly_affiliation_fee ?? null, platform_fee_monthly ?? null, req.params.id]);
  ...
  router.post("/orders", async (req: Request, res: Response) => {
    try {
      ...
      const seq = String((cnt[0].n ?? 0) + 1).padStart(4,"0");
  ...
  router.post("/ledger/calculate", async (req: Request, res: Response) => {
    try {
      ...
      // 司機薪資成本（fleet_driver_payroll，有 fleet_id + month + net_salary）
  ...
  // GET /fleet-system/ledger/summary/all — 所有車隊損益彙總（必須在 :fleetId 前）
