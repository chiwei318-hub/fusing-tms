...
export default function OwnerDashboard() {
  ...
  const addDriver = useMutation({
    mutationFn: () => apiFetch(`/affiliated-owners/${ownerId}/drivers`, {
      ...
      body: JSON.stringify({
        ...dForm,
        vehicle_id: dForm.vehicle_id || null,
  ...
  const confirmStmt = useMutation({
    mutationFn: () => apiFetch(
      `/affiliated-owners/${ownerId}/statement/${currentStmt?.id}/confirm`,
      { method: "PATCH" }
    ),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["owner-statement"] });
      toast({ title: "✅ 月結單已確認" });
    },
  });
          ...
          <div style={S.grid2}>
            ...
            <div style={S.card}>
              ...
              {receivables.length > 0 ? receivables.map((r: any, i: number) => (
                <FR key={i}
                  label={`${r.route_type ?? "趟次"} (${r.trip_count}趟)`}
              ...
              <div style={{ display:"flex", justifyContent:"space-between",
                padding:"10px 12px",
                background: netPayout >= 0 ? "#0a1f14" : "#1f0a0a",
                borderRadius:8,
                border: `1px solid ${netPayout >= 0 ? "#166534" : "#7f1d1d"}` }}>
              ...
              {currentStmt?.status === "draft" && (
                <button
                  style={{ ...S.addBtn, marginTop:10, width:"100%", padding:"8px" }}
                  onClick={() => confirmStmt.mutate()}
                  disabled={confirmStmt.isPending}>
                  {confirmStmt.isPending ? "確認中…" : "✅ 確認月結單"}
                </button>
              )}
              ...
              <div style={{ ...S.card, marginBottom:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  <FF label="車牌號碼 *" value={vForm.plate_no}
                    onChange={(v: string) => setVForm(p => ({...p, plate_no: v}))} />
                  <div>
                    <FL>車輛類型</FL>
                  ...
                  <FF label="廠牌" value={vForm.vehicle_brand}
                    onChange={(v: string) => setVForm(p => ({...p, vehicle_brand: v}))} />
                  <FF label="年份" value={vForm.year} type="number"
                    onChange={(v: string) => setVForm(p => ({...p, year: v}))} />
                  <FF label="最大載重(kg)" value={vForm.max_load_kg} type="number"
                    onChange={(v: string) => setVForm(p => ({...p, max_load_kg: v}))} />
                  <FF label="保險到期日" value={vForm.insurance_expiry} type="date"
                    onChange={(v: string) => setVForm(p => ({...p, insurance_expiry: v}))} />
                  <FF label="驗車到期日" value={vForm.inspection_expiry} type="date"
                    onChange={(v: string) => setVForm(p => ({...p, inspection_expiry: v}))} />
                </div>
                ...
                <div key={v.id} style={{ ...S.card, padding:"14px 16px" }}>
                  ...
                  {v.vehicle_brand && (
                    <div style={{ fontSize:12, color:"#64748b" }}>
                      {v.vehicle_brand}{v.year ? ` (${v.year}年)` : ""}
                    </div>
                  )}
                  {v.insurance_expiry && (
          ...
          <div>
              ...
              <div style={{ ...S.card, marginBottom:16 }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10 }}>
                  <FF label="司機姓名 *" value={dForm.driver_name}
                    onChange={(v: string) => setDForm(p => ({...p, driver_name: v}))} />
                  <FF label="手機號碼" value={dForm.driver_phone}
                    onChange={(v: string) => setDForm(p => ({...p, driver_phone: v}))} />
                  <FF label="駕照號碼" value={dForm.license_no}
                    onChange={(v: string) => setDForm(p => ({...p, license_no: v}))} />
                  <div>
                    <FL>指定車輛</FL>
                  ...
                  {dForm.pay_type === "per_trip" ? (
                    <FF label="每趟費率（元）" value={dForm.per_trip_rate} type="number"
                      onChange={(v: string) => setDForm(p => ({...p, per_trip_rate: v}))} />
                  ) : (
                    <FF
                      label={dForm.pay_type === "daily" ? "日薪（元）" : "月薪（元）"}
                      value={dForm.base_pay} type="number"
                      onChange={(v: string) => setDForm(p => ({...p, base_pay: v}))} />
                  )}
                </div>
                <div style={{ display:"flex", gap:8, marginTop:14 }}>
                  <Btn color="#1e293b" textColor="#64748b"
            ...
            <Tbl
              ...
              rows={drivers.map((d: any) => [
                ...
                <SBadge key={d.id} status={d.is_active ? "active" : "inactive"} />,
              ])}
              empty="尚未新增司機"
            />
          </div>
          ...
          <Tbl
            ...
            rows={statements.map((s: any) => [
              ...
              toNum(s.penalty_deduct) > 0
                ? <span key={s.id} style={{ color:"#ef4444" }}>{fmt(toNum(s.penalty_deduct))}</span>
                : "—",
              <strong key={s.id}
                style={{ color: toNum(s.net_payout) >= 0 ? "#4ade80" : "#f87171" }}>
                {fmt(toNum(s.net_payout))}
              </strong>,
              <SBadge key={s.id + "s"} status={s.status} />,
            ])}
            empty="尚無歷史記錄"
          />
        )}
