...
*   **富詠全智慧物流清算平台（6 大模組）：** 模組1 廠商管理（`partners` 表，CRUD 合約費率，`GET/POST/PUT/DELETE /api/partners`），模組2 車型矩陣（`vehicle_type_matrix` + `vehicle_equipment` 表，`/api/vehicle-matrix/types|equipment`），模組3 智慧報價引擎（`POST /api/smart-quote/calculate|detect-zone`，zone 地帶 × 車型因子 × 設備加成），模組4 AR/AP 財務清算（`order_financials` 表 + trigger backfill，`/api/financials`），模組5 廠商查價入口（`/quote/:partnerId?t=<base64url-token>`，`quotePortalRouter`），模組6 月結 Excel 導出（ExcelJS 6 工作表，`GET /api/financials/export-excel?period=YYYY-MM`）。前端新增：`/partners`（廠商管理）、`/financials`（財務清算儀表板）、`/ar-ap`（AR/AP 帳務）、`/quote/:id`（廠商查價入口）。
