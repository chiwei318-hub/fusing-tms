...
*   **Granular Status Flow & Exception SOP (COMPLETED):** Expanded order status machine: `pending→assigned→arrived→loading→in_transit→delivered`, plus `exception` state (with resolve). New DB columns: `arrived_at`, `loaded_at`, `exception_code`, `exception_note`, `exception_at`, `exception_attribution`, `pod_photo_url`, `pod_note`. New table: `order_status_history` (full audit trail per status change). New backend routes (orderStatusFlow.ts): `POST /api/orders/:id/status-event` (events: arrive/start_loading/exception/resolve_exception/pod_upload), `GET /api/orders/:id/timeline` (full timeline JSON), `GET /api/orders/:id/status-history`, `GET /api/exception-codes` (12 reason codes E01–E99), `GET /api/exception-stats`. Exception code reference (12 codes): E01=客戶不在現場, E02=貨物未備妥, E03=地址錯誤, E04=超重超尺寸, E05=道路塞車, E06=車輛故障, E07=氣候, E08=司機健康, E09=貨物損毀, E10=交通事故, E11=等候費, E99=其他. Admin LINE push on exception. Driver portal updated: 5 new status-conditional button sections (到點打卡, 開始裝貨, 出發配送, 回報異常 dialog with reason code selector, 繼續配送), exception state panel. `OrderStatusTimeline` component shows step-by-step progress bar with timestamps, exception banner, POD photo, and expandable history. `StatusBadge` extended for: arrived(司機到點), loading(裝貨中), exception(異常).
*   **Dispatch Suggestion Engine & Fleet Analytics (COMPLETED):** `GET /api/dispatch/suggest?orderId=X` ranks drivers by multi-factor scoring (status +20, vehicle type +30, cargo type +15, service area +20, workload -8×active, rating +10, productivity +8). Returns top-N with match_level (strong/moderate/weak) and reason tags. `DispatchSuggestPanel` admin component: shows ranked drivers with score, reasons, one-click assign. `GET /api/analytics/demand-forecast`: hourly pattern, day-of-week, monthly trend, top routes from 90-day history. `GET /api/analytics/fleet-recommendation`: vehicle type utilization, outsource ratio, coverage gaps, AI recommendations. `GET /api/exception-stats`: exception type distribution by code + attribution. New admin tab "預測" (DemandForecastTab) with 4 sub-tabs: 需求預測, 車隊建議, 異常分析, 派車引擎.
