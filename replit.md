...
- **路線報價** — Route-based fixed price table (origin → destination × vehicle type); DB-backed CRUD; seeded with PDF data; Excel export
- **車輛成本** — Per-vehicle cost calculator: fuel, depreciation, license/fuel tax, insurance, maintenance, wear, salary, other; auto-calculates fixed cost/month + fuel cost/trip by distance; color bar breakdown chart; summary stats; Excel export
