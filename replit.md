...
*   **福興高車隊帳號系統 (Fleet Sub-contractor) — 4-layer settlement chain:** Admin portal `/fusingao` → 4 tabs: 車趟完成通知, 月度對帳, 合作車隊管理 (with commission_rate/bank fields), 結算總覽 (per-fleet settlement breakdown). Fleet portal `/fleet` → 5 tabs: 可搶路線 (atomic grab), 我的任務 (mark complete + assign driver dropdown), 月結帳單 (CSV export), 旗下司機 (CRUD fleet drivers), 結算分析 (settlement chain visualization + per-driver breakdown). DB tables: `fusingao_fleets` (with commission_rate/bank_account/bank_name), `fleet_drivers`, `orders.fusingao_fleet_id/fleet_driver_id/fleet_grabbed_at/fleet_completed_at`. APIs: `GET/POST/PUT /api/fusingao/fleets/:id/drivers`, `GET /api/fusingao/settlement`, `GET /api/fusingao/fleets/:id/settlement`, `PUT /api/fusingao/routes/:id/assign-driver`. Auth: `POST /api/auth/login/fleet` JWT, role `fusingao_fleet`. Settlement formula: shopee_rate → platform keeps commission_rate% → fleet receives remainder. Test account: fleet01/test1234.
